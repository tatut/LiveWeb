...
CMD sleep 1; rlwrap ./pharo --headless -- eval "ZnServer startDefaultOn: 8080. ZnServer default delegate map: #counter to: [ :req | (LWExamplePage of: LWMultiCounter) value: req ]; map: #clock to: [:req | (LWExamplePage of: LWClockExample) value: req ]; map: #wordle to: [:req | (LWExamplePage of: LWWordle) value: req ]; map: #quit to: [ :_ | Smalltalk quitPrimitive ]."